...
```{r eval=TRUE}
#Task: FILL IN THE CODE BELOW and review the outputs
library(tidyverse)
...
- the underscore versions of read_csv are part of the tidyverse package
2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
- Shows class of vector: "data.frame". Output: "spec_tbl_df" "tbl_df"      "tbl"         "data.frame" 
3. How many columns does the object have when created with these different functions?
- kings 1 and 2 has 1 column, kings 3 and 4 has 9 (checked with the ncol() function)
4. Show the dataset so that we can see how R interprets each column
- run kings shows dataset as a tibble 55x9
...
kings$start_of_reign <- as.numeric(kings$start_of_reign)
kings$end <- as.numeric(kings$end_of_reign)
...
kings <- kings %>% 
  ...
  select(start_of_reign,end_of_reign, name_monach) %>% 
  mutate(duration = end_of_reign-start_of_reign)

```

Task: Calculate the average duration of reign for all rulers
...
- First i make a vector with c() function, then i convert it to numeric so as to be able to run mean() function to calculate average rule. Answer: approx 20.3
...
#- a vector, which i convert to numeric with function "duration <-as.numeric(kings$duration)"
...
```{r}
duration <-as.numeric(kings$duration)
...
Question: How many and which kings enjoyed a longer-than-average duration of reign?
- 24 kings reigned for longer than average (20.3 yrs)
...
- I created a new tibble which shows the number (24), name and duration of kings with above average reign time

```{r}
kings %>% 
  select(name_monach, duration) %>% 
  filter(duration > 20.3)


```


Question: How many days did the three longest-ruling monarchs rule? 
- The three longest ruling monarchs ruled for 60 (Christian_4), 52 (margrethe_2) and 43(Erik_7) years respectively
